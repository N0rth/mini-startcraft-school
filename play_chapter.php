...
*/		
		...
		$user_session = array( 
								array("slug_name"=>$slug_name,"name"=>$name,"user"=>get_user_values($user),"point"=>50,"challenge"=>$challenges_list,"item"=>get_user_item($challenges_list))
							 );
...
{	
		...
		#var_dump($challenge_property_data);		
		...
		//get user session details
		$getUsers = array_merge(array(array_values(get_session($seesionKey))),array(array('challenge_property'=>$challenge_property_data['property'],'challenge_value'=>$challenge_property_data['value'])));
